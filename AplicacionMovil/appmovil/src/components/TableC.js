...
const TableC = () => {
  ...
  const fetchData = () => {
    fetch(`http://${ip}:5000/allcompanies`)
            ...
            try {
              const response = await fetch(`http://${ip}:5000/deletedataoftable`, {
                ...
                body: JSON.stringify({
                  table: 'company',
                  id: keytransport, 
                  key: 'nombre' // Corregido: Quita el espacio al final
                }),
              });
  ...
  return (
    <View style={styles.table}>
      ...
      {products.all_companies && products.all_companies.map((product) => (
        <View style={styles.row} key={product.nombre}>
          <Text style={styles.cell}>{product.nombre}</Text>
          <Text style={styles.cell}>{product.direccion}</Text>
          <Text style={styles.cell}>{product.telefono}</Text>
          <Text style={styles.cell}>{product.correo_electronico}</Text>
          ...
          <View style={styles.cell}>
            ...
            <TouchableOpacity onPress={() => handleDeleteUser(product.nombre)}>
                <View style={styles.btn}>
